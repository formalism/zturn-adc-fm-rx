...
(
	...
	output signed[DATA_WIDTH+2-1:0]		i_out,		// DATA_WIDTH+2 bits
...
reg signed [DATA_WIDTH+2-1:0]					r_i_out, r_q_out;
	...
	always @(posedge clk) begin
		...
		if (r_i[SINUSOIDAL_WIDTH+DATA_WIDTH-1] == 1'b1 &&
			r_i[SINUSOIDAL_WIDTH+DATA_WIDTH-2:SINUSOIDAL_WIDTH+DATA_WIDTH-BIT_SHIFT] != 
					{(BIT_SHIFT-1){1'b1}})
			r_i_out	<=	{1'b1, {(DATA_WIDTH+1){1'b0}}};
		else if (r_i[SINUSOIDAL_WIDTH+DATA_WIDTH-1] == 1'b0 &&
			r_i[SINUSOIDAL_WIDTH+DATA_WIDTH-2:SINUSOIDAL_WIDTH+DATA_WIDTH-BIT_SHIFT] != 
					{(BIT_SHIFT-1){1'b0}})
			r_i_out	<=	{1'b0, {(DATA_WIDTH+1){1'b1}}};
		else
			r_i_out	<=	r_i[SINUSOIDAL_WIDTH+DATA_WIDTH-BIT_SHIFT:SINUSOIDAL_WIDTH-BIT_SHIFT-1];

		if (r_q[SINUSOIDAL_WIDTH+DATA_WIDTH-1] == 1'b1 &&
			r_q[SINUSOIDAL_WIDTH+DATA_WIDTH-2:SINUSOIDAL_WIDTH+DATA_WIDTH-BIT_SHIFT] != 
					{(BIT_SHIFT-1){1'b1}})
			r_q_out	<=	{1'b1, {(DATA_WIDTH+1){1'b0}}};
		else if (r_q[SINUSOIDAL_WIDTH+DATA_WIDTH-1] == 1'b0 &&
			r_q[SINUSOIDAL_WIDTH+DATA_WIDTH-2:SINUSOIDAL_WIDTH+DATA_WIDTH-BIT_SHIFT] != 
					{(BIT_SHIFT-1){1'b0}})
			r_q_out	<=	{1'b0, {(DATA_WIDTH+1){1'b1}}};
		else
			r_q_out	<=	r_q[SINUSOIDAL_WIDTH+DATA_WIDTH-BIT_SHIFT:SINUSOIDAL_WIDTH-BIT_SHIFT-1];
	end
